...
def _value_to_rgb(vals: np.ndarray) -> np.ndarray:
    """Return Nx3 float array in [0,1] (green->yellow)."""
    if vals.size == 0:
...
def _downsample(n: int, max_n: int) -> np.ndarray:
    ...
    if idx.size > max_n: idx = idx[:max_n]
    return idx
...
class FukaWorldEdges3D(ThreeDScene):
    def construct(self):
        ...
        _pad         = _env_float("FUKA_PAD", 0.5)  # reserved
        ...
        # bounds + recenter (avoid camera.frame on 0.19)
        if sx.size:
        ...
        # static camera angle (works in 0.19)
        ...
        for fi in range(F):
            ...
            if n_s:
                ...
                for j in range(xs.shape[0]):
                    ...
                    # Use HEX to avoid numpy array pitfalls in ManimColor
